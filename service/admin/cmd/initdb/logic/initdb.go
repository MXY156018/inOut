...
type TypedDBInitHandler interface {
	EnsureDB(ctx context.Context, conf *InitDB) (context.Context, error) // 建库，失败属于 fatal error，因此让它 panic
...
func (initDBService *InitDBService) InitDB(conf *InitDB) (err error) {
	ctx := context.TODO()
