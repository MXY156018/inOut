...
var (
	Discard = New(log.New(ioutil.Discard, "", log.LstdFlags), LogConfig{}, nil)
...
func New(writer logger.Writer, cfg LogConfig, zapLog *zap.Logger) logger.Interface {
	var (
		infoStr      = "%s\n[info] "
		warnStr      = "%s\n[warn] "
		errStr       = "%s\n[error] "
...
func NewZapLog(cfg LogConfig, zapLog *zap.Logger, level logger.LogLevel) logger.Interface {
	l := New(nil, cfg, zapLog)
	l.LogMode(level)
	return l
}
...
func (c *_logger) Printf(message string, data ...interface{}) {
	...
	if c.zapLog != nil {
		level := c.zapLog.Level()
		if level <= zap.InfoLevel {
			c.zapLog.Info(fmt.Sprintf(message, data...))
		} else {
			c.zapLog.Error(fmt.Sprintf(message, data...))
		}
	}
}
