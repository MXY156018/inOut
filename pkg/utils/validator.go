...
func Verify(st interface{}, roleMap Rules) (err error) {
	...
	if kd == reflect.Struct {
		return verifyStruct(st, roleMap)
	} else if kd == reflect.Ptr {
		if val.Elem().Kind() != reflect.Struct {
			return fmt.Errorf("参数需要结构体类型,实际为 %s", val.Elem().Kind().String())
		}
		return verifyStructPtr(st, roleMap)
	} else {
		return fmt.Errorf("参数需要结构体类型,实际为 %s", val.Elem().Kind().String())
	}
}

func verifyStruct(st interface{}, roleMap Rules) error {
...
func verifyStructPtr(st interface{}, roleMap Rules) error {
	compareMap := map[string]bool{
		"lt": true,
		"le": true,
		"eq": true,
